...
from stuff.models import fastText
...
int_sents = h5py.File(indir + 'unigram_ints.hdf5', mode='r')
...
if optimize:
    ...
    def evaluate_hps(dropout,
        ...
        params = np.array([dropout, patience,
                           e_size, batch_size, learning_rate])
    ...
    pd.Series(best).to_csv(outdir + 'best_ft_bg_params.csv', index=False)
...
best = np.array(pd.read_csv(outdir + 'best_ft_bg_params.csv',
                            header=None)).flatten()
...
test_guesses.to_csv(indir + 'ft_guesses.csv', index=False)
#stats.to_csv(indir + 'ft_f1_stats.csv', index=False)
